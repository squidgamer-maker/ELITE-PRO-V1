    async function isAdmin(EliteProTech, chatId, senderId) {
        try {
            const groupMetadata = await EliteProTech.groupMetadata(chatId);
            
            const botId = EliteProTech.user.id.split(':')[0] + '@s.whatsapp.net';
